...
**Cloud/DevOps** [[GitHub](https://github.com/jarviscanada/jarvis_data_eng_KevinShimotakahara/tree/master/cloud_k8_jenkins)]: This project used Microsoft Azure Kubernetes Service (AKS) in conjunction with Jenkins pipelines to deploy the aforementioned SpringBoot Microservice Stock Trading App on the cloud, and have it maintained with Continuous Integration/Continuous Development (CI/CD) pipelines. Jenkins was provisioned on a Minikube cluster locally, where it could automatically clone the git repository containing the application's codebase; build, test, and containerize the application by running Dockerfiles; push Docker images containing instances of our application's microservices to an Azure Container Repository (ACR); and use kubectl commands to update pod deployments in our AKS cluster with ACR images. Both development (dev) and production (prod) environments were created with their own AKS clusters and CI/CD pipelines to practice DevOps paradigms.
